...
def choose_best(frequencies: dict, candidates: tuple) -> str:
    ...
    for e in candidates:
        if e in frequencies:
            new_dict[e] = frequencies[e]  # это те кандидаты, которые есть в словаре частот
            new_list_values.append(frequencies[e])  # это список из частот тех кандидатов, которые есть в словаре частот

    max_freq = max(new_list_values)

    for e in new_dict:
        if frequencies[e] == max_freq:
            top_cand.append(e)  # это список тех кандидатов, которые обладают максимальной частотой
            top_cand.sort()
...
def spell_check_word(frequencies: dict, as_is_words: tuple, word: str) -> str:
    ...
    corr_candidates = keep_known(candidates, frequencies)
    best_cand = choose_best(frequencies, corr_candidates)
